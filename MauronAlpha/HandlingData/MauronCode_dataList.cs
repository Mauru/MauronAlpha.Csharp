using System.Collections.Generic; using System;  namespace MauronAlpha.HandlingData {  	public class MauronCode_dataList<T>:MauronCode_dataObject,ICollection<T>,IEnumerable<T> { 		//constructor 		public MauronCode_dataList() {}  		//Data
		...
		bool ICollection<T>.Remove (T item) {
			...
			Remove(item);
			return true;
		}  		#endregion  		IEnumerable<int> ENUM_element=(IEnumerable<int>)L_data; 		            		public IEnumerator<T> GetEnumerator ( ) { 			for(int i=0; i<L_data.Count; i++) 			yield return i; 		}  		System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator ( ) { 			throw new NotImplementedException(); 		} 	}  } 