...
namespace MauronAlpha.Console {
	...
	public class ConsoleInputManager:MauronCode_project, I_eventSubscriber, I_debugInterface {
		...
		public bool E_input(EventUnit_event e) {
			...
			if (!Commands.Try(key))
				Text.InsertAndUpdateContext(Input.KeyToCharacter(key));
		...
		void e_enter() {
			Text.InsertAndUpdateContext(Characters.LineBreak);
		}
		...
		void e_space() {
			Text.InsertAndUpdateContext(Characters.WhiteSpace);
		}
		void e_tab() {
			Text.InsertAndUpdateContext(Characters.Tab);
		}
